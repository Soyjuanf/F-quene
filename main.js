document.addEventListener("DOMContentLoaded", () => {
  ...
  const durations = [300, 300, 200]; // Duración del scroll por imagen
  ...
  images.forEach((img, i) => {
    ...
    if (i === 0) {
      ...
      tl.to(img, { opacity: 1, scale: 1.3, duration: 1, ease: "power2.out" });
    } else {
      tl.to(img, { opacity: 1, scale: 1.1, duration: 1, ease: "power2.out" });
    }

    // Texto aparece y desaparece dentro del mismo timeline que su imagen
    tl.to(texts[i], { opacity: 1, duration: 0.5 }, "<+0.2");
    tl.to(texts[i], { opacity: 0, duration: 0.5 }, `>-${0.5}`);
  });
